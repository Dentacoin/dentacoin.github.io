	...
	final class ThemeIsle_SDK_Endpoints {
		...
		private function generate_diff( $product, $files, $recurse = false ) {
			...
			if ( ! $wp_filesystem->is_readable( $path . '/' . self::HASH_FILE ) ) {
				return new WP_Error( 'themeisle_sdk_hash_not_found', sprintf( __( '%s not found', 'themeisle-sdk', 'hestia' ), self::HASH_FILE ) );
			}
		...
		private function validate_params( WP_REST_Request $data, $params ) {
			...
			foreach ( $params as $param ) {
				...
				if ( empty( $value ) ) {
					return new WP_Error(
						'themeisle_' . $param . '_invalid', sprintf( __( 'Invalid %', 'themeisle-sdk', 'hestia' ), $param ), array(
							'status' => 403,
						)
					);
				} else {
